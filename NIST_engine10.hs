...
main = 
  do
    ...
    bmwrite (model ++ ".bmp") $ bmvstack $ map (\bm -> bminsert (bmempty ((11*3)+2) (((11*3)+2)*9)) 0 0 bm) $ map (bmhstack . map (\(_,hrs) -> bmborder 1 (hrbm 11 3 2 (hrs `hrhrred` vvk)))) $ qqll $ treesPaths $ hrmult uu1 df1' hr'
    printf "bitmap %s\n" $ model ++ ".bmp"
    hFlush stdout
