...
hrconcat = vectorHistoryRepasConcat_u . V.fromList
...
main = 
  do
    ...
    (uu,hr1) <- nistTrainBucketedRegionRandomIO 2 15 17
    (uu,hr2) <- nistTrainBucketedRegionRandomIO 2 15 7
    (uu,hr3) <- nistTrainBucketedRegionRandomIO 2 15 11
    (uu,hr4) <- nistTrainBucketedRegionRandomIO 2 15 13

    let hr = hrconcat [hr1,hr2,hr3,hr4]
    ...
    let model = "NIST_model32"
    let (wmax,lmax,xmax,omax,bmax,mmax,umax,pmax,fmax,mult,seed) = (2^11, 8, 2^9, 30, (30*3), 3, 2^8, 1, 255, 1, 5)
    ...
    let hr' = hrev (take 7500 [i | i <- [0..], i `mod` 8 == 0]) hr
    ...
    bmwrite (model ++ "_2.bmp") $ bmvstack $ map (\bm -> bminsert (bmempty ((15*2)+2) (((15*2)+2)*(maximum (map length pp)))) 0 0 bm) $ map (bmhstack . map (\((_,ff),hrs) -> bmborder 1 (bmmax (hrbm 15 2 2 (hrs `hrhrred` vvk)) 0 0 (hrbm 15 2 2 (qqhr uu vvk (fund ff)))))) $ pp
    printf "bitmap %s\n" $ model ++ "_2.bmp"
    hFlush stdout
