...
main = 
  do
    ...
    let df1' = dfred uu1 (df1 `dflt` 16) hr
...
{-
    ...
    hFlush stdout
-}
    bmwrite (model ++ ".bmp") $ bmvstack $ map (\bm -> bminsert (bmempty (28+2) ((28+2)*9)) 0 0 bm) $ map (bmhstack . map (\(_,hrs) -> bmborder 28 (hrbm 28 28 1 0 2 (hrs `hrhrred` vvk)))) $ qqll $ treesPaths $ hrmult uu2 df2' hr'
    printf "bitmap %s\n" $ model ++ ".bmp"
    hFlush stdout
