...
w = stringsVariable "<<0,2>,1>"
ff0 = qqff $ sgl $ trans (unit (llqq [llss [(u,ValInt 0),(w,ValInt 0)], llss [(u,ValInt 1),(w,ValInt 1)]])) (sgl w)
ss0 = llss [(w,ValInt 1)]
...
main = 
  do
    ...
    s <- BL.readFile "NIST_model11.json"
    let df1a = fromJust $ persistentsDecompFud $ fromJust $ (Data.Aeson.decode s :: Maybe DecompFudPersistent)
    let df1 = lldf $ map (\((_,ff1):ll) -> (stateEmpty,ff0):((ss0,ff1):ll)) $ dfll df1a

    let uu1 = uu `uunion` (fsys (dfff df1))

    let ff1 = fframe (refr1 2) $ fromJust $ systemsDecompFudsNullablePracticable uu1 df1 2
    ...
    let model = "NIST_model13"
    let (kmax,omax) = (1, 5)
    let (mult,seed) = (1, 5)

    printf ">>> %s\n" $ model
    hFlush stdout
    let (uu2,df2a) = decompercondrr vvl uu1 hr1 kmax omax 127
    let df2 = zzdf $ funcsTreesMap (\(ss,ff) -> (ss, (ff `funion` gg1) `fdep` fder ff)) $ dfzz df2a
