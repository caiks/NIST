...
main = 
  do
    ...
    bmwrite (model ++ ".bmp") $ bmvstack $ map (\bm -> bminsert (bmempty ((9*3)+2) (((9*3)+2)*(maximum (map length pp)))) 0 0 bm) $ map (bmhstack . map (\(_,hrs) -> bmborder 1 (hrbm 9 3 8 (hrs `hrhrred` vvk)))) $ pp
    printf "bitmap %s\n" $ model ++ ".bmp"
    hFlush stdout
